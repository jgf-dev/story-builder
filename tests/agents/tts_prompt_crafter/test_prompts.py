...
def test_get_prompt_not_found(tmp_path):
    with patch("storybuilder.agents.tts_prompt_crafter.prompts.PROMPTS_DIR", str(tmp_path)):
        with pytest.raises(FileNotFoundError, match="Prompt 'missing_prompt' not found at"):
            get_prompt("missing_prompt")

def test_get_story_success(tmp_path):
...
def test_get_story_not_found(tmp_path):
    with patch("storybuilder.agents.tts_prompt_crafter.prompts.STORIES_DIR", str(tmp_path)):
        with pytest.raises(FileNotFoundError, match="Story 'missing_story' not found at"):
