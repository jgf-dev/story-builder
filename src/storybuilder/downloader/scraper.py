...
def scrape_subcategory(sub_url, start_date, end_date, delay, force_scan=False):
    ...
    while current_url:
        ...
        for row in rows:
            ...
            if not force_scan and not is_directory and story_date < start_date:
                safe_print(f"Reached story with date {story_date} which is older than start_date {start_date}. Stopping traversal.")
                stop_pagination = True
                break
    ...
    with cache_lock:
        metadata_cache[sub_url] = {"last_updated": datetime.datetime.now().isoformat(), "complete": is_complete or reached_end, "stories": merged_stories}
    ...
    for s in merged_stories:
        ...
        if in_range:
            filtered_stories.append({"name": s["name"], "url": s["url"], "date": s_date, "is_dir": s["is_dir"]})
