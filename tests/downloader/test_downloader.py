...
class TestScraperSubcategories(unittest.TestCase):
    @unittest.mock.patch("storybuilder.downloader.scraper.fetch_page")
    def test_get_subcategories_success(self, mock_fetch):
        ...
        # HTML with list-group-item and external/parent directory to filter out
        html = """
        <html>
            <ul>
                <li class="list-group-item"><a href="adult-friends/">Adult Friends</a></li>
                <li class="list-group-item"><a href="another-folder/">Another Folder</a></li>
                <li class="list-group-item"><a href="../">Parent Directory</a></li>
            </ul>
        </html>
        """
        mock_fetch.return_value = MockResponse(html)

        results = get_subcategories("gay", delay=0)

        self.assertEqual(len(results), 2)
        self.assertEqual(results[0]["name"], "Adult Friends")
        self.assertEqual(
            results[0]["url"], "https://nifty.org/nifty/gay/adult-friends/"
        )
        self.assertEqual(results[1]["name"], "Another Folder")
        self.assertEqual(
            results[1]["url"], "https://nifty.org/nifty/gay/another-folder/"
        )

        mock_fetch.assert_called_once()

    @unittest.mock.patch("storybuilder.downloader.scraper.fetch_page")
    def test_get_subcategories_fallback(self, mock_fetch):
        from storybuilder.downloader.scraper import get_subcategories
        ...
        # HTML without list-group-item, but with anchor tags that end in '/'
        html = """
        <html>
            <body>
                <a href="adult-friends/">Adult Friends</a>
                <a href="another-folder/">Another Folder</a>
                <a href="http://external.com/">External Link</a>
                <a href="/nifty/lesbian/">Other Category</a>
                <a href="../">Parent</a>
                <a href="not-a-folder.txt">Not a folder</a>
            </body>
        </html>
        """
        mock_fetch.return_value = MockResponse(html)
    ...
    @unittest.mock.patch("storybuilder.downloader.scraper.fetch_page")
    def test_get_subcategories_empty(self, mock_fetch):
        from storybuilder.downloader.scraper import get_subcategories
