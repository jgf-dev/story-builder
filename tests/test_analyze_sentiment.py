...
def test_get_sentiment_value():
    assert (
        analyze_sentiment.get_sentiment_value({"label": "positive", "score": 0.8})
        == 0.8
    )
    assert (
        analyze_sentiment.get_sentiment_value({"label": "NEGATIVE", "score": 0.8})
        == -0.8
    )
    assert (
        analyze_sentiment.get_sentiment_value({"label": "neutral", "score": 0.8}) == 0.0
    )
