...
sys.path.insert(
    0,
    str(
        Path(__file__).resolve().parents[1] / ".agent/skills/tts-prompt-crafter/scripts"
    ),
)
...
from storybuilder.downloader.db import init_db, insert_story, close_db, get_conn
...
class TestSplitPrompts(unittest.TestCase):
    ...
    def test_split_line_to_sentences_no_split_in_brackets(self):
        ...
        self.assertEqual(
            sentences[0],
            "Jace: [sighs, whispering closely. soft pace] I remember everything.",
        )
