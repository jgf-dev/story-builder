...
## 2024-07-24 - HTML5 sections as semantic landmarks
**Learning:** HTML5 `<section>` elements do not act as semantic `region` landmarks for screen reader navigation by default. They need structural links to existing visible headers to provide semantic context.
**Action:** Upgrade complex structural regions like cards or groups by using `aria-labelledby` and `aria-describedby` to structurally link them to their existing visible `<header>` tags (e.g., `<h2>` and `<p>`), providing richer semantic context than a standalone `aria-label`.
...
## 2024-11-20 - Prefer Native List Elements
**Learning:** When creating grouped content like stats sections, relying on generic `<div>` containers with `role="group"` is less semantically rich for screen readers. Using native `<ul>` and `<li>` elements provides better built-in navigation and grouping announcements.
**Action:** Always prefer native HTML lists (`<ul>`/`<ol>`) over generic containers with ARIA list or group roles. Default list styles can be cleanly reset via CSS (`list-style: none`, `margin: 0`, `padding: 0`).
