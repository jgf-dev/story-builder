...
def fetch_story(conn, story_query=None):
    if story_query:
        df_story = pd.read_sql_query("SELECT id, story_dir FROM stories WHERE story_dir LIKE ?", conn, params=(f"%{story_query}%",))
    else:
...
def get_overall_sentiment(conn, story_id, window):
    ...
    df_sentences["smoothed_sentiment"] = df_sentences["sentiment_score"].rolling(
        window=window, center=True, min_periods=max(1, window // 10)
    ).mean()
    return df_sentences
