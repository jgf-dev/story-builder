...
class TestRotateWindscribeIp(unittest.TestCase):
    ...
    def test_rotate_windscribe_ip_success(self, mock_run, mock_sleep, mock_safe_print):
    ...
    @patch("storybuilder.downloader.network.safe_print")
    @patch("subprocess.run")
    def test_rotate_windscribe_ip_failure(self, mock_run, mock_safe_print):
        mock_result = MagicMock()
    ...
    @patch("storybuilder.downloader.network.safe_print")
    @patch("subprocess.run")
    def test_rotate_windscribe_ip_exception(self, mock_run, mock_safe_print):
